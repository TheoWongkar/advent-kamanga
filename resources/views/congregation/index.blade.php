                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-center mb-6 space-y-4 md:space-y-0">
                        ...
                        <div class="flex space-x-2 w-full md:w-1/3">
                            <form action="{{ route('jemaat.index') }}" method="GET" class="flex w-full">
                                <input type="text" name="search" value="{{ $search }}"
                                    class=" px-4 py-2 w-full rounded-l-full border border-[#141652] focus:ring-0 focus:border-blue-800"
                                    placeholder="Cari jemaat..." autocomplete="off" autofocus />
                                <button type="submit"
                                ...
                                <tbody>
                                    @foreach ($congregations as $congregation)
                                        <tr class="border-t hover:bg-blue-100 transition duration-200">
                                            ...
                                            <td class="py-4 px-2 sm:px-4">{{ $congregation->name }}</td>
                                            <td class="py-4 px-2 sm:px-4 text-center">{{ $congregation->gender }}</td>
                                            <td class="py-4 px-2 sm:px-4 text-center">{{ $congregation->age }}</td>
                                            <td class="py-4 px-2 sm:px-4 text-center">{{ $congregation->address }}</td>
                                            <td class="py-4 px-2 sm:px-4 text-center">
                                                {{ $congregation->created_at->format('d M Y') }}
                                            </td>
                                            <td class="py-4 px-2 sm:px-4 flex space-x-2 justify-center">
                                                <a href="{{ route('jemaat.edit', $congregation->id) }}"
                                                ...
                                                <form action="{{ route('jemaat.destroy', $congregation->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button
