<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Ubah Jadwal : ') }} <span class="text-blue-800">{{ substr($worship->title, 0, 20) }}</span>
        </h2>
    </x-slot>
                    ...
                    <form action="{{ route('ibadah.update', $worship->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="preacher" id="preacher" required
                                ...
                                value="{{ $worship->preacher = old('preacher') ?? $worship->preacher }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="date" id="date" required
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:border focus:border-[#141652]"
                                placeholder="Masukkan pengkhotbah"
                                value="{{ $worship->date = old('date') ?? $worship->date }}">
